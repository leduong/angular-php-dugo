var App=angular.module("App","ngAnimate ngTouch ngSanitize ngRoute App.filters App.services App.directives App.controllers ui.bootstrap google-maps".split(" ")).value("version","\u00a9 2001-2014 nhadat.com").value("Domain","").config(["$routeProvider","$locationProvider","$httpProvider",function(a,b,d){a.when("/intro.html",{templateUrl:"intro.html",controller:"IntroCtrl"});a.when("/login.html",{templateUrl:"auth/login.html",controller:"LoginCtrl"});a.when("/update.html",{templateUrl:"auth/update.html"});
...
b=h(d);var l=a[0].createElement("img");l.onload=function(){g&&g.remove();angular.isDefined(e.lazyLoadingClass)&&c.removeClass(e.lazyLoadingClass);angular.isDefined(e.lazyLoadedClass)&&c.addClass(e.lazyLoadedClass);c.css({"background-image":"url("+this.src+")"})};l.onerror=function(){};l.src=b})}}}]).directive("tiFocus",function(){return{restrict:"A",link:function(a,b,d){var c=b.find("input");b.bind("click",function(){c[0].focus()})}}}).directive("tiAutosize",function(){return{restrict:"A",require:"ngModel",
link:function(a,b,d,c){var e,f;e=angular.element('<span class="input"></span>');e.css("display","none").css("visibility","hidden").css("width","auto").css("white-space","pre");b.parent().append(e);f=function(a){var c=a,f;angular.isString(c)&&0===c.length&&(c=d.placeholder);c&&(e.text(c),e.css("display",""),f=e.prop("offsetWidth"),e.css("display","none"));b.css("width",f?f+3+"px":"213px");return a};c.$parsers.unshift(f);c.$formatters.unshift(f);d.$observe("placeholder",function(a){c.$modelValue||f(a)})}}});
angular.module("App.filters",[]).filter("unsafe",["$sce",function(a){return function(b){return a.trustAsHtml(b)}}]).filter("fixbr",function(){return function(a){if(a)return a.replace(/&lt;br(.*?)\/&gt;/g,"<br />")}}).filter("thumb",function(){return function(a){return-1<["jpg","jpeg","png","gif","bmp"].indexOf(/(?:\.([^.]+))?$/.exec(a)[1])?a:"attach.png"}}).filter("bbcode",function(){return function(a){if(a){for(var b=[/\[b\](.*?)\[\/b\]/gi,/\[i\](.*?)\[\/i\]/gi,/\[u\](.*?)\[\/u\]/gi,/\[3d\](.*?)\[\/3d\]/gi],
