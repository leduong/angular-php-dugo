...
class tuh {
	...
	public static function unparse($t,$d=TRUE) {
		...
		foreach($t as $p)
			if($c=1-$c)
				$s.=str_replace(array('<p>','</p>','<br />'),'',($d?tuh::unh($p):$p));
			else
				$s.=tuh::uncode($p);
		return $s;
	}
	public static function to_utf8($t) {
		if(!preg_match('/[^\x00-\x7F]/S',$t)) {
			$e=error_reporting(~E_NOTICE);
	...
	protected static function text($t,$d=TRUE) {
		$s='';
		foreach(explode("\n\n",$t) as $l) {
			$l=tuh::quote(tuh::link($l));
			if($d)
				$l=tuh::decode(tuh::h($l));
			$l=tuh::close($l);
